...
pChoice <- data.frame(expand.grid(list(
  Preference = c(0, 1),
  Knowledge = c(0, 1),
  Character = c(0, 1),
  Start = c(0, 1),
  ...
  mutate(
    ...
    Knowledge = factor(Knowledge, levels = c(0, 1), labels = c('No', 'Yes')),
    Start = factor(Start, levels = c(0, 1), labels = c('Pizza', 'Hotdog')), # Flipped now, after Oct2023
    Path = factor(Path, levels = c(0, 1), labels = c('Short', 'Long')),
    Choice = factor(Choice, levels = c(0, 1), labels = c('Pizza', 'Hotdog')),
    p_long = NA,
    p_hotdog = NA,
    p_action = NA
  ) # p_long, used to be p_short
...
pChoice$p_hotdog <- l2p(
  hd_b0 +
    ...
    hd_bCS *
      (as.numeric(pChoice$Character) - 1) *
      (as.numeric(pChoice$Start) - 1) +
    hd_bSK *
      (as.numeric(pChoice$Start) - 1) *
      (as.numeric(pChoice$Knowledge) - 1)
)
...
pChoice <- pChoice %>%
  mutate(
    p_action = if_else(
      pChoice$Path == "Long" & pChoice$Choice == "Hotdog",
      p_long * p_hotdog,
      if_else(
        pChoice$Path == "Long" & pChoice$Choice == "Pizza",
        p_long * (1 - p_hotdog),
        if_else(
          pChoice$Path == "Short" & pChoice$Choice == "Hotdog",
          (1 - p_long) * p_hotdog,
          (1 - p_long) * (1 - p_hotdog)
        )
      )
...
# check they sum to 1
pChoice %>%
  group_by(Knowledge, Preference, Character, Start) %>%
...
pChoice <- pChoice %>%
  mutate(
    Z = if_else(pChoice$Preference == "Absent", "0", "1"),
...
# And concatenate
pChoice <- pChoice %>%
  unite("numtag", Z:U, sep = "", remove = TRUE)
