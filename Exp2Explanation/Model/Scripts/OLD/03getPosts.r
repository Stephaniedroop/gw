...
# Cn get active edges from active_path and active_food
...
u_combos_path <- as.data.frame(expand.grid(replicate(
  n_u_path,
  c(0, 1),
  simplify = FALSE
)))
...
u_combos_food <- as.data.frame(expand.grid(replicate(
  n_u_food,
  c(0, 1),
  simplify = FALSE
)))
...
pChoice_path$PrUn <- apply(
  pChoice_path[, grepl("_var$", colnames(pChoice_path))],
  1,
  prod
)
...
pChoice_path$PrUn <- pChoice_path$PrUn *
  ifelse(
    pChoice_path$Path == "Short",
    pChoice_path$p_short,
    pChoice_path$p_long
  )

# Do same for FOOD
pChoice_food$PrUn <- apply(
  pChoice_food[, grepl("_var$", colnames(pChoice_food))],
  1,
  prod
)
...
pChoice_food$PrUn <- pChoice_food$PrUn *
  ifelse(
    pChoice_food$Food == "Pizza",
    pChoice_food$p_pizza,
    pChoice_food$p_hotdog
  )
...
save(
  path_post,
  food_post,
  file = here('Exp2Explanation', 'Model', 'Data', 'posteriors.rda')
)
