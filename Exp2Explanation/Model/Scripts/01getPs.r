...
pChoice <- data.frame(
  situationVerbose = rep(df.m$situationVerbose, each = 4),
...
pChoice <- pChoice |>
  mutate(
    p_action = ifelse(
      Food == "Pizza" & Path == "Short",
      p_short * p_pizza,
      ifelse(
        Food == "Pizza" & Path == "Long",
        p_long * p_pizza,
        ifelse(
          Food == "Hotdog" & Path == "Short",
          p_short * p_hotdog,
          p_long * p_hotdog
        )
      )
...
pChoice <- pChoice |>
  mutate(
    Food = factor(Food, levels = c("Pizza", "Hotdog")),
