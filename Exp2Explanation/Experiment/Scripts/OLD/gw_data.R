...
coded_CM$OtherRat <- as.numeric(
  coded_CM$c_Disposition |
    coded_CM$c_Situation |
    coded_CM$c_Food.disp |
    coded_CM$c_Char.now
) # not rowSums(coded_CM[ , c(8, 9)])
...
df <- df %>%
  unite("tag", stim1, stim4, sep = "", remove = FALSE)

# And now we want a slick skinny df of what we will need to attach model predictions to ####### TO DO
ugdata <- df %>%
  dplyr::select(completionCode, tag, Cong, response, PreferenceRat:OtherRat)
