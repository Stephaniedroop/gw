...
idat <- read.csv(
  here('Exp2Explanation', 'Experiment', 'Data', 'coder1.csv'),
  na.strings = c("", " ", "NA")
)
vdat <- read.csv(
  here('Exp2Explanation', 'Experiment', 'Data', 'coder2.csv'),
  na.strings = c("", " ", "NA")
)
...
# Use letters to make less unwieldy but match same order as the category names from schema
# (which is: P, C, K, S, Pu, Cu, Ku, Su) [Later note: base vars in different order from when generated model predictions]
...
a <- c(2:9)
idat[, a] <- apply(idat[, a], 2, function(x) as.numeric(as.character(x)))
...
save(
  ...
  vdat2,
  cats,
  file = here('Exp2Explanation', 'Experiment', 'Data', 'ratings.Rda')
)
