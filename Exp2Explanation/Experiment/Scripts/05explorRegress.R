...
run_glmers <- function(
  ...
  for (out_col in output_cols) {
    ...
    df_sub <- data[
      !is.na(data[[out_col]]),
      c(input_cols, participant_col, out_col)
    ]
    # Build formula string
    formula_str <- paste0(
      out_col,
      " ~ ",
      paste(input_cols, collapse = " + "),
      " + (1 | ",
      participant_col,
      ")"
    )
...
# Now run the glmers!
glmer_results <- run_glmers(
  rated_explans,
  input_cols,
  output_cols,
  participant_col
) # converegence warnings
...
data_means <- data_sums |>
  summarise(
    across(total_a:total_unc, ~ mean(.x), .names = "mean_{.col}"),
    .groups = 'drop'
  )
...
data_sd <- data_sums |>
  summarise(
    across(total_a:total_unc, ~ sd(.x), .names = "sd_{.col}"),
    .groups = 'drop'
  )
