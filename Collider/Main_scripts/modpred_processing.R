...
# Older - was working, before I did the 10x reoptimised model preds

# Model predictions and conditional probabilities - not yet multiplied

all <- read.csv('../model_data/all.csv') %>% replace(is.na(.), 0) # 4032 of 19. now 20160 of 25

# Pivot_longer_prepare: Later we need to pivot_longer two sets of columns at once, so rename columns
# all <- all %>% rename(A_cp = 3, Au_cp = 4, B_cp = 5, Bu_cp = 6, 
#                       structure = 7, pgroup = 9,
#                       vA = 10, vAu = 11, vB = 12, vBu = 13)
