...
# Then puts it in merged ppt and model dfs, split by probability group (1:6) and structure (c/d)
...
# and model data
mp <- read.csv('../model_data/tidied_preds.csv')


# What if we don't split, and just put each model prediction next to each row? This would use just data and mp
data <- data %>% rename(node3 = ansVar3, pgroup=probgroup)
data <- data %>% select(1,8:10,13:21)
data <- data %>% select(1,12,6,5,2:4,7,11,13,8:10)


comb <- merge(x=data, y=mp, by=c('pgroup', 'trialtype', 'node3'), all.x = TRUE)

# THIS NOT WORKING YET. ANYWAY, BELOW THIS IS FROM OLD SCRIPT SO NOT USABLE RIGHT NOW




# Now decide how to slice the mp big df, for to best compare with the ppts. Maybe slice into 12?
