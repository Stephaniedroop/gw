...
# Once we got it working, we ran again with base rate {.5,.5} 
pA <- c(0.5,0.5) # 
peA <- c(.9,.1) #... most of the time the noise var for a doesn't occur. for a to work it needs a and exp a. 
pB <- c(0.5,0.5) #  
peB <- c(.2,.8) # Note for toy version we can't have extreme values because sometimes all resample is false and then sd is zero
...
save(params, dfd, dfc, mp1c, mp1d, file='collider1.Rdata') # ie collider 1 is with pA and pB base rate .5,.5
