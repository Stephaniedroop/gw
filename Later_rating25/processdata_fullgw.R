###################################################################
##### Gridworld data processing and some merging with ratings #####

# The participant data is here but does not need much processing. 
# The most important thing is to tag the ratings with what condition they refer to

# Previously this project was called 'ug data rerun' so that name crops up sometimes
library(tidyverse)
library(lme4)
#library(brms)
#library(utiml) #utilities for multi label learning
library(broom.mixed)
...
# There is a mistake in the data where 3 participants completed it twice, giving good faith answers on two differnet occasions
# Need to decide what to do about these: remove from all? Use different ID? 
# Decided to reallocate the second attempt of each as follows:
...
# If we decide instead to remove these participants' data, wait til it has been combined with the ratings
# Go to section [XX] below, uncomment the line starting [xxx], and run that code before proceeding.
